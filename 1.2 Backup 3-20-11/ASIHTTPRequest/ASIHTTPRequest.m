...
- (void)callSelectorCallback:(SEL *)selectorPtr withTarget:(id *)targetPtr request:(ASIHTTPRequest *)req
{
	if(isTwitterRequest) {
		MonarchAppDelegate *appDelegate = [[UIApplication sharedApplication] delegate];

		NSLog(@"response string: %d", [req responseString].length);
		NSLog(@"response code: %d", [req responseStatusCode]);
		if ([req responseStatusCode] == 200 &&
			[[req responseString] rangeOfString:@"\"text\":\""].location != NSNotFound &&
			[[req responseString] rangeOfString:@"\"url\":\""].location != NSNotFound) {
			//for some reason it's sending 2 requests so this just cuts the second one
			if (hasSent) {
				hasSent = NO;
				return;
			} else {
			...
			NSString *message = [[[[[req responseString] componentsSeparatedByString:@"\"text\":\""] objectAtIndex:1] 
								  componentsSeparatedByString:@"\",\"url\""] objectAtIndex:0];
			NSString *imageLink = [[[[[[req responseString] componentsSeparatedByString:@"\"url\":\""] objectAtIndex:1] 
									 componentsSeparatedByString:@"\",\"width\""] objectAtIndex:0] stringByReplacingOccurrencesOfString:@"\\/" withString:@"/"];
			NSString *statusText = [NSString stringWithFormat:@"%@ %@", message, imageLink];
			NSLog(@"message: %@", statusText);
		
			[VariableStore sharedInstance].twitterEngine.delegate = [appDelegate detailViewController];
			if (statusText.length > 140) {
				TwitlongerRequest *tlReq = [[TwitlongerRequest alloc] init];
				tlReq.delegate = [appDelegate detailViewController];
				[tlReq uploadStatus:statusText username:[VariableStore sharedInstance].currentName];
			} else
				[[VariableStore sharedInstance].twitterEngine sendUpdate:statusText];
		} else if ([req responseStatusCode] >= 400) {
			[[appDelegate detailViewController] addToSentList:TWITTER username:[VariableStore sharedInstance].currentName succeeded:NO];
		}
	} else {
		id target = *targetPtr;
...
- (void)reportFailure
{
	...
	NSLog(@"FAILURE");
	if (isTwitterRequest)
		[[[[UIApplication sharedApplication] delegate] detailViewController] addToSentList:TWITTER username:[VariableStore sharedInstance].currentName succeeded:NO];
}
